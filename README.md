...
The index-structure shootout, the concurrency and read-protocol
experiments, the memory-backing tiers, the in-process comparison with
`cachedb_local` and `cachedb_redis`, the end-to-end topology-hiding runs
at 50k and 100k held calls, the huge-page arena and what the soak and
production caught live next to the module, with their graphs:
[modules/cachedb_perf/doc/STUDY.md](https://github.com/Lt-Flash/opensips/blob/feature/cachedb-perf-devel/modules/cachedb_perf/doc/STUDY.md)
on the `feature/cachedb-perf-devel` branch. This README covers what came
after it: the cluster-scale pull-sharing protocol, the allocators and the
cache's memory backings.
