/* VENDORED from Lt-Flash/perfcached 7baf2d9 (lib/perfd.c).
...
{
	char *ps = params_ck(col, key, NULL), *r;
	int v = 0;
...
{
	char *ps = params_ckp(col, key, path ? path : "$", NULL), *r;
	struct pc_jtok *t;
	int ntok, found = 0, tv;
...
int perfd_jdel(perfd_t *p, const char *col, const char *key,
		const char *path)
{
	char *ps = params_ckp(col, key, path, NULL), *r;
	int v = 0;
		...
		const perfd_opts *opts)
{
	...
	for (i = 0; i < (nsec ? nsec : 1); i++) {
		struct timeval tv = { io_ms / 1000, (io_ms % 1000) * 1000 };
